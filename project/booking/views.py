...
def movie_details(request, movie_id):
	...
	try:
		...
		shows = Show.objects.filter(movie=movie_id).order_by('theatre')#date=datetime.date.today()
		qs=list(shows)
		show_list = []
		show_by_theatre = []
		theatre = shows[0].theatre
		for i in range(0,len(shows),1):
	...
	return render(request, 'movie/movie_details.html',
		{'movie_info': movie_info, 'show_list': show_list,'qs':qs})
